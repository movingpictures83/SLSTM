...
class SLSTMPlugin:
   ...
   def run(self):
    ...
    # GRU PIPELINE
    model = create_LSTM_model('Sequential', int(self.parameters["units"]), train_X, train_y)
    train_LSTM(model, float(self.parameters["lr"]), int(self.parameters["epochs"]), train_X, train_y, test_X, test_y)
